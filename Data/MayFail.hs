...
newtype MayFail e f t = MayFail {
    getMayFail :: Compose (Either e) f t
...
instance Alternative f => Alternative (MayFail e f) where
    ...
    x <|> y = MayFail . Compose $ case (x', y') of
      ...
      where
        x' = getCompose . getMayFail $ x
        y' = getCompose . getMayFail $ y
...
instance (Functor f, Monad f, CommutesWithEither f) => Monad (MayFail e f) where
    ...
    x >>= k = MayFail . Compose $ case getCompose . getMayFail $ x of
        Left e -> Left e
        Right fterm -> case commuteEither . fmap (getCompose . getMayFail . k) $ fterm of
            Left e' -> Left e'
...
runMayFail :: MayFail e f t -> Either e (f t)
runMayFail = getCompose . getMayFail
